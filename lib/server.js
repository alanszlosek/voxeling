...
function Server(config, serverSettings, clientSettings) {
...
Server.prototype.initialize = function() {
    ...
    self.coords = Coordinator(self.config.chunkSize);

    // Must pass this in, for now
    self.generator = self.serverSettings.generator;
    self.chunkCache = self.config.chunkCache || {};
...
Server.prototype.bindClientEvents = function(client) {
    ...
    connection.on('myPosition', function(position) {
    ...
    connection.on('chunkVoxelIndexValue', function(details) {
        // Make sure this is a number
        var chunkID = details[0];
        if (chunkID in self.chunkCache) {
            var chunk = self.chunkCache[chunkID];
            delete self.encodedChunkCache[chunkID];
            for (var i = 1; i < details.length; i += 2) {
                var index = details[i];
                var val = details[i + 1];
                ...
                self.broadcast(client.id, 'chunkVoxelIndexValue', details);
...
Server.prototype.sendInitialChunks = function(connection) {
    var self = this;
    var chunkIDs = [];
    // Could send a smaller amount of chunks on first connect
    self.coords.nearbyChunkIDsEach(self.clientSettings.worldOrigin, this.clientSettings.horizontalDistance, this.clientSettings.verticalDistance, function(chunkID) {
        chunkIDs.push(chunkID);
    });
    self.sendChunks(connection, chunkIDs);
...
Server.prototype.sendChunks = function(connection, chunkIDs) {
    ...
    for (var i = 0; i < chunkIDs.length; i++) {
        var chunkID = chunkIDs[i];
        var pair = self.prepareChunkForSending(chunkID);
        if (pair) {
            out.push(pair);
        }
    }
