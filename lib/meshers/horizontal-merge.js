// NOTE: aye, need more indentation cleanup.

/*
Mesher works, but it's fucking slow!
Revisits many voxels, re-allocates lots
*/
...
var addFace = function(basePosition, face, info) {
    ...
    switch (face) {
      ...
      case 'left':
        ...
        texcoord.append([ 0, 0, endZ - startZ + 1, 0, endZ - startZ + 1, endY - startY + 1, 0, 0, endZ - startZ + 1, endY - startY + 1, 0, endY - startY + 1 ]);
        break;
      ...
      case 'top':
        ...
        texcoord.append([ 0, 0, endX - startX + 1, 0, endX - startX + 1, endZ - startZ + 1, 0, 0, endX - startX + 1, endZ - startZ + 1, 0, endZ - startZ + 1 ]);
        break;
...
var isFaceBlocked = function(voxels, chunkSize, face, x, y, z) {
    if (debug) {
    ...
    for (var z = 0; z < chunkSize; z++) {
        for (var y = 0; y < chunkSize; y++) {
            ...
            for (var x = 0; x < chunkSize; x++, index++) {
                var voxelTextureValue = voxels[index];
                ...
                for (var face in adjacent) {
                    var textureValue;
                    var isBlocked = isFaceBlocked(voxels, chunkSize, face, x, y, z);
                    if (debug) {
...
var Meshing = module.exports = {
    config: function(cs, textures, coordinatorHandle) {
        chunkSize = cs;
