...
- keep a pool of buffer bundles (buffers for positions, texcoords, normals)
- when a chunk changes, remove it from the current bundle, appending it to the end of a bundle that has space
...
Voxels.prototype.getBufferBundle = function() {
    ...
    var obj;
    /*
    How much buffer space should we pre-allocate?

    - Worst case is a voxel every-other spot (roughly 16 x 16 x 16)
    - Six points per side
    - Six sides
    ...
    For texcoords
    - 2 float32s per point (4 bytes per float32)
    ...
    */
    ...
    if (!obj) {
        obj = {
            ...
            // How many tuples this buffer holds currently
            tuples: 0
        };

        // Need to multiply size by 4 since we're dealing with Float32 values
...
Voxels.prototype.addVoxelMesh = function(id, mesh) {
    ...
    // TODO: might be nice if the mesh kept track of the exact number of tuples it contains
    // this would allow us to more accurately test whether a mesh can be appended to a buffer
...
Voxels.prototype.removeChunkMesh = function(chunkId) {
    if (chunkId in this.buffersByChunk) {
        ...
        if (Object.keys(buffer.chunks).length == 0) {
            // Buffer has been fully emptied, allow it to be re-filled from it's beginning
            buffer.tuples = 0;
        }
    }
};
