...
Physics.prototype.tick = function() {
    ...
    if (currentVelocity[1] == 0) {
        ...
        if (this.controlState.jump) {
            ...
            acceleration = accelerations.fly;
        } else {
            acceleration = accelerations.gravity;
        }
    } else if (this.controlState.fly) {
        acceleration = accelerations.fly;
    } else {
        if (jumped && currentVelocity[1] < 0) {
            acceleration = accelerations.partialGravity;
        } else {
            acceleration = accelerations.gravity;
        }
    }
    if (this.controlState.fly) {
        currentVelocity[1] += acceleration;
        if (currentVelocity[1] > 0) {
