...
class Chunk {
    constructor(runtime, config, chunkStore, generator) {
        this.log = runtime.log('Chunk');
        this.config = config;
    ...
    read(chunkId, chunkPosition) {
        ...
        if (this.reads.has(chunkId)) {
            self.log('read', 'Returning promise');
            return this.reads[chunkId].promise;
        }
        self.log('read', 'Creating new promise');
        let o = {
    ...
    changeBlocks(changes) {
        let self = this;
        self.log('changeBlocks', 'Got changes');
        ...
        this.chunkStore.read(chunkId, readInfo.chunkPosition).then(
            function(chunk) {
                ...
                if (chunkId in self.chunkChanges) {
                    self.log('_read', 'Applying block changes after read');
            ...
            function(error) {
                ...
                // generate then save
                self.log('_read', 'Generating because ' + error);
                let chunk = {
