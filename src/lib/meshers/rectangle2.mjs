...
2020-01-19
using index offset based tests in skipFace() only saves a few milliseconds. bummer. but maybe looping over axes based on stepping instead of having to calculate the index based on x,y,z will save more
*/
...
class RectangleMesher {
    constructor(config, voxToTex, texOffsets, coordinates, cache) {
        ...
        this.lastVoxel = config.chunkWidth - 1; // last in row
        this.lastVoxel2 = config.chunkWidth2 - config.chunkWidth; // last row in cube
        this.testZ = (config.chunkWidth * config.chunkWidth * config.chunkWidth) - config.chunkWidth2;
                ...
                for (let x = 0; x < this.chunkWidth; x++) {
                    ...
                    for (let faceIndex in faces) {
                        ...
                        let skip = (
                            ...
                            ||
                            this.skipFace(voxels, x, y, z, index, bitwiseFace)
                        );
                        ...
                        if (this.visited[index] & bitwiseFace) {
                            ...
                            continue;
                        } else if (this.skipFace(voxels, x, y, z, index, bitwiseFace)) {
                        ...
                        while (tryColumn || tryRow) {
                            while (tryColumn) {
                                ...
                                switch (bitwiseFace) {
                                    ...
                                    case this.bitwiseFaces.top:
                                    case this.bitwiseFaces.bottom:
                                        shouldSkipFace = this.skipFace(voxels, newEndColumn, y, z, index2, bitwiseFace);
                                        break;
                                    case this.bitwiseFaces.left:
                                    case this.bitwiseFaces.right:
                                        shouldSkipFace = this.skipFace(voxels, x, y, newEndColumn, index2, bitwiseFace);
                                        break;
                                }
                            ...
                            while (tryRow) {
                                ...
                                for (let col = startColumn; col <= endColumn; col++) {
                                    ...
                                    switch (bitwiseFace) {
                                        case this.bitwiseFaces.back:
                                        case this.bitwiseFaces.front:
                                            shouldSkipFace = this.skipFace(voxels, col, newEndRow, z, index2, bitwiseFace);
                                            break;
                                        case this.bitwiseFaces.top:
                                        case this.bitwiseFaces.bottom:
                                            shouldSkipFace = this.skipFace(voxels, col, y, newEndRow, index2, bitwiseFace);
                                            break;
                                        case this.bitwiseFaces.left:
                                        case this.bitwiseFaces.right:
                                            shouldSkipFace = this.skipFace(voxels, x, newEndRow, col, index2, bitwiseFace);
                                            break;
                                    }
    ...
    skipFace(voxels, x, y, z, index, face) {
        switch (face) {
            case this.bitwiseFaces.back:
                if (z == 0) {
